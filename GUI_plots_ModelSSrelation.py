...
import GUI_functions_ModelSSrelation as fn
import numpy as np
...
def run_bedload(all_data, units, a=0,b=0,c=0) :
    ...
    if units == 1:        # Convert English units to SI units for all calcs in GUI
        ...
        gs_cols  = gsd['Bins'][1:len(gsd)-1].astype(str)
...
def optimize_beta(): 
    ...
    # Optimize beta
    beta = 5600                                     # initial beta
    meas_sum = bedload_data['Bedload'].sum()        # sum of all measured bedload
    phi_table = pd.DataFrame()                      # phi = tau * i / tau * ri
    SS_prop = pd.DataFrame()                        # dataframe of shear stress proportions from gamma dist
    total_load = pd.DataFrame()                     # sum of all predicted bedload for all measured discharges
    SS_dist_values = pd.DataFrame({'Discharge': bedload_data['Discharge']})   # Discharge, SS, and SS proportions to be exported
    
    # First, get phi = tao_i / tao_ri for every grain size and every discharge
    # ss = shear stresses in the distribution, ss_prop = proportion in distribution
    for ss in np.arange(1000):
        SS_prop = fn.SS_from_dist(stage_depth['Depth alpha'], stage_depth['Avg depth'], bedload_data['Slope'], ss)
        SS_dist_values[f'Prop (SS = {ss})'] = SS_prop
        for i in np.arange(len(SS_prop)):
            if SS_prop[i] < 1E-3:
                SS_prop[i] = 0
        
        for i in np.arange(len(grain_sizes)):
            tau_i = ss / (1000 * 9.81 * (2.65-1) * (grain_sizes['GS'][i]/1000))
            tau_ri = HF_a *(grain_sizes['GS'][i] / D50)**HF_b
            phi_table[f"{str(gsd_bins['Bins'][i+1])}"] = np.repeat(tau_i / tau_ri, len(bedload_data))
        ...
        q_bi = Wi_table.mul(F_i.values) * ss**(3/2) / ((2.65 - 1) * 9.81 * 1000**(3/2))
        q_bi_ss = q_bi.mul(SS_prop, axis=0)
        total_load = total_load.fillna(0) + q_bi_ss
        
    ratio = meas_sum / total_load.sum().sum()
    ...
    names = list(SS_dist_values.columns)
    np.savetxt("SS_dist_values.csv", SS_dist_values, delimiter=",", header = ','.join(names), comments='')     # Save shear stress values and proportions
...
def run_model(all_data, units1, units2, a=0, b=0, c=0):
    ...
    for ss in np.arange(1000):
        ...
        model_bedload = model_dimless_bedload.mul(F_i.values) * ss**(3/2) / ((2.65 - 1) * 9.81 * 1000**(3/2))
        
        meas_model_q = meas_model_q.fillna(0) + model_bedload.mul(SS_prop, axis=0)
    ...
    for ss in np.arange(5000):
        SS_prop = fn.SS_from_dist(stage_depth['Depth alpha'], stage_depth['Avg depth'], flow_freq['Slope'], ss)
        for i in np.arange(len(SS_prop)):
            if SS_prop[i] < 1E-3:
                SS_prop[i] = 0
        ...
        for i in np.arange(len(grain_sizes)):
            tau_i = ss / (1000 * 9.81 * (2.65-1) * (grain_sizes['GS'][i]/1000))
            tau_ri = HF_a *(grain_sizes['GS'][i] / D50)**HF_b
            phi_table[f"{str(gsd_bins['Bins'][i+1])}"] = np.repeat(tau_i / tau_ri, len(flow_freq))

        model_dimless_bedload = ref_bedload * optimized_beta.iloc[0,0] * (1 - (0.853/phi_table))**4.5
        
        model_bedload = model_dimless_bedload.mul(F_i.values) * ss**(3/2) / ((2.65 - 1) * 9.81 * 1000**(3/2))
        
        total_model_load = total_model_load.fillna(0) + model_bedload.mul(SS_prop, axis=0)
    ...
    labels = ['Total Bedload flux', '1-to-1']
